...
【計算步驟】
1. 計算用戶所有未使用券的總額
2. 如果消費金額 ≥ 券總額，建議用盡所有券：實付 = 消費 - 券總額
3. 如果消費金額 < 券總額，優先用大面額券，建議用能覆蓋消費金額的券組合

【回答格式】
消費 XXX 元，使用所有可用券（總額 YYY 元）：
實付 = XXX - YYY = ZZZ 元

【舉例】
用戶有 510 元券，消費 1836 元：
實付 = 1836 - 510 = 1326 元

用戶有 100 元券，消費 80 元（不夠最低消費 100×3=300）：
無法使用此券
